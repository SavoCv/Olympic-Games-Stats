...
bool was_medal;
map <const Games*, set <pair<const Competitor*, pair <const Event*, string>>>> mapGamesCompetitorsAndEventsAndMedal;
map <const Games*, set <const Event*>> mapGamesEvents;
...
void add(const smatch& sm, bool is_team = false)
{
	...
	co->set(g, e, sm[7], c);
	mapGamesCompetitorsAndEventsAndMedal[g].insert(make_pair(co, make_pair(e, sm[7])));
	mapGamesEvents[g].insert(e);
}
...
set <const Competitor*> filter()
{
	...
	return handle_filter(sport, country, medal, year, type);
}
